...
public final class ImageUtil {
     ...
     * @return base64字符串
     */
    public static synchronized String imageToBase64ByLocal(String imgPath) {

        if (StringUtils.isBlank(imgPath)) {
            throw new UncheckedException(ErrorCode.PARAM_NULL, "本地图片的地址不能为空");
        }
        // 返回Base64编码过的字节数组字符串
     ...
     * @return base64字符串
     */
    public static synchronized String imageToBase64ByLocal(File image) {

        if (null == image || !image.exists()) {
            throw new UncheckedException(ErrorCode.PARAM_NULL, "本地图片不存在");
        }
        ...
        try {
            ...
            throw new UncheckedException(ErrorCode.PARSE_ERROR, e.getMessage());
        } finally {
     ...
     * @return 输出图片
     */
    public static synchronized File base64ToImage(String imgBase64Str, String imagePath) {
        if (!StringUtils.isNoneBlank(imgBase64Str, imagePath)) {
            throw new UncheckedException(ErrorCode.PARAM_NULL, "输入参数错误");
        }
     ...
     * @return 输出图片
     */
    public static synchronized File base64ToImage(String imgBase64Str, File img) {
