...
public class StreamUtil {
    ...
    public synchronized static <T> List<T> remove(List<T> list, Predicate<? super T> predicate) {
        ...
        List<T> collect = list.parallelStream().filter(Objects::nonNull).filter(predicate.negate()).collect(Collectors.toList());
        return collect;
    }
    ...
    public synchronized static <T> Set<T> remove(Set<T> set, Predicate<? super T> predicate) {
        ...
        Set<T> collect = set.parallelStream().filter(Objects::nonNull).filter(predicate.negate()).collect(Collectors.toSet());
        return collect;
    }
