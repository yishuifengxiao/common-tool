 ...
 * @since 1.0.0
 */
public class TLVUnit implements Serializable {
    private String data;
    private String tag;
    ...
    /**
     * 将给定的标签和输入字符串转换为标签-长度-值（TLVUnit）格式的字符串。
     * 该函数根据输入字符串的长度（以字节为单位）选择合适的长度编码方式。
     *
     * @param tag   表示 TLVUnit 格式中的标签部分，为字符串类型
     * @param value 表示 TLVUnit 格式中的值部分，应为十六进制字符串，每两个字符代表一个字节
     * @return 返回一个符合 TLVUnit 格式的字符串
     */
    public static String toTLV(String tag, String value) {
        return TLV.toTLV(tag, value);
    }
     ...
     *
     * @param value 表示 TLVUnit 格式中的值部分，应为十六进制字符串，每两个字符代表一个字节
     * @return 返回一个符合 TLVUnit 格式的字符串，不包含标签部分
     */
    public static String toTLV(String value) {
        return TLV.toTLV(null, value);
    }
     ...
     */
    // 静态工厂方法
    public static TLVUnit of(String data) {
        return new TLVUnit(data);
    }
     ...
     */
    // 统一的解析方法 - 支持链式调用
    public TLVUnit parse(String tag) {
        // 如果当前对象已经成功解析并且有值部分，则从值部分继续解析（链式解析）
        if (success && !value.isEmpty()) {
            return new TLVUnit(value).parse(tag);
        }
     ...
     */
    public TLVUnit parseRemaining(String tag) {
        if (!success || remainingData.isEmpty()) {
            // 修复：当无法解析剩余数据时，返回一个新的失败TLV对象而不是当前对象
            TLVUnit failedTlvUnit = new TLVUnit(remainingData);
            failedTlvUnit.setError("没有剩余数据可用于解析");
            failedTlvUnit.remainingData = this.remainingData; // 确保remainingData正确传递
            failedTlvUnit.value = ""; // 修复：确保value字段不为null
            return failedTlvUnit;
        }
        return new TLVUnit(remainingData).parse(tag);
    }
